 ...
 //el.rate >= this.state.r)
  render(){return (
    ...
    <Star rates={this.state.r}  ratechercher={(y)=>this.ratesearch(y) } />   
     <Liste isloading={this.state.isloading} detail={this.state.info.filter(el =>   (el.name.toUpperCase().includes( this.state.keywords.toUpperCase().trim()))  && (el.rate >= this.state.r)) }/>
